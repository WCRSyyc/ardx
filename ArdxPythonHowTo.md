...
To do all of the LABs in the Arduino ARDX experimenter’s kit, but instead of using C++ we will substitute circuitPython.

## <a name="block_materials">⚓</a> Materials

* an ARDX kit
* the ADRX experiment instructions [https://wcrsyyc.github.io/ardx/starthere.html](https://wcrsyyc.github.io/ardx/starthere.html)
...
* use a web browser to go to [circuitPython.org](https://circuitPython.org), find your board and download the latest stable version of CircuitPython.
...
If you haven't already installed the MU editor/IDE on your Laptop/Workstation do that now.
(you can use other editors and other serial terminals but this one is the easiest.)
Download MU here:  [https://codewith.mu](https://codewith.mu)
...
You can edit the file in the micro controller disk drive directory, but don’t forget to save a copy in your computer’s local drive when you are finished for the day.

## <a name="block_labs">⚓</a> Doing the LABs

Look at each LAB in the Arduino ARDX experimenters guide and setup a similar hardware configuration except for substituting the Arduino with a Python capable micro controller and using Python code instead of the Arduino version of C++. You may also have to change the pin names in some of the LABs.

Also note that the ARDX experimenter’s guide assumes a 5 volt Arduino but most of the Python capable micro controllers have a 3.3 volt VCC. The I/O pins on most 3.3 volt micro controllers are not 5 volt tolerant, so be careful that you do not connect then to any signal or power supply higher than 3.3 volts.
...
[the code](code/CIRC01p-code.txt)

[the code](code/CIRC01.py)
...
You will have to copy the file folder adafruit_motor from the library files you extracted in setup above to the lib folder on your micro controller.
...
Use a separate power supply for the servo. Remember to connect the ground from the separate power supply to the ground of the CircuitPythonExpress. The servo’s power supply should be 5 or 6 volts.

Did you get: ImportError: no module named 'adafruit_motor'?

There is not room for all of the library files available on this small board so you will have copy individual library files as needed.

You will have to copy the file folder adafruit_motor from the library files you extracted in setup above to the lib folder on your circuitPythonExpress.
